...
class PlatformDispatcherService {  
    ...
    dispatcherAction(action,serviceType) { 
        try { console.log('[UI-Dispatch] dispatcherAction ->', serviceType, action && action.actionType, action && action.data); } catch(_) {}
    ...
    processEvents(event,serviceType) {
        try { console.log('[UI-Dispatch] processEvents <-', serviceType, event && event.event, event && event.data); } catch(_) {}
        if(serviceType == CommonConstants.SERVICE_TYPE_MQTT_CLIENTS) {
