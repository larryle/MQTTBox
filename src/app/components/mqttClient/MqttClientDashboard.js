...
class MqttClientDashboard extends Component {
    ...
    changeConnectionState() {
        try { console.log('[UI-Click] changeConnectionState, current=', this.state.conState, 'mcsId=', this.props.params.mcsId); } catch(_) {}
        if(this.state.conState == MqttClientConstants.CONNECTION_STATE_CONNECTED) {
            try { console.log('[UI-Click] dispatch DISCONNECT for', this.props.params.mcsId); } catch(_) {}
            MqttClientActions.disConnectFromBroker(this.props.params.mcsId);
        } else {
            try { console.log('[UI-Click] dispatch CONNECT with settings=', this.state.mqttClientSettings); } catch(_) {}
    ...
    updateConnectionStatus(connStateObj) {
        try { console.log('[UI] updateConnectionStatus called with', connStateObj, 'page.mcsId=', this.props.params.mcsId); } catch(_) {}
        if(connStateObj.mcsId == this.props.params.mcsId) {
            try { console.log('[UI] applying state update for', connStateObj.mcsId); } catch(_) {}
    ...
    componentDidMount() {
        ...
        // 进入详情页自动连接（若未连接）
        try {
            var current = MqttClientService.getMqttClientStateByMcsId(this.props.params.mcsId);
            if (current !== MqttClientConstants.CONNECTION_STATE_CONNECTED && this.state && this.state.mqttClientSettings) {
                try { console.log('[UI] auto-connect on mount for', this.props.params.mcsId); } catch(_) {}
                MqttClientActions.connectToBroker(this.state.mqttClientSettings);
            }
        } catch(_) {}
    }
    ...
    render() {
        ...
        try { console.log('[UI-Render] current conState=', this.state.conState, 'mcsId=', this.props.params.mcsId); } catch(_) {}
